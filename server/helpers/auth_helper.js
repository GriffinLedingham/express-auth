...
module.exports = function (models, language) {
  ...
  auth_Helper.signupUser = function(username, password, email) {
    var response = this.helpers.general.getErrorMessagePromise(language.signup.missingFields);
    if(typeof username != 'undefined'
      && typeof password != 'undefined'
      && typeof email != 'undefined' ) {
      response = models.user.createUser(username, password, email);
    }
    return response;
  };
  ...
  auth_Helper.getSession = function(sid) {
    return models.session.findOne({where:{sid: sid}});
  };

  auth_Helper.comparePassword = function(password, user) {
    var that = this;
    var result = {txn: false, err: language.login.wrongPassword};
    if(user) {
      result = bcrypt.compareAsync(password, user.get('password'))
      .then(function(res) {
        var returnData = {};
        if(res === true) {
